...
def main(path_classes_file = "classes.txt"):
    """Convert data to COCO format."""
    # imgs_dir, annots_dir, output_file = sys.argv[1], sys.argv[2], sys.argv[3]
    imgs_dir, annots_dir, output_file = r".\Data\Fruits\images", \
                                        r".\Data\Fruits\annotations",\
                                        r".\Data\Fruits\data.json"
    with open(path_classes_file, "r") as file:
        sp = file.readlines()
    #category2id = {"banana": 1, "snake fruit": 2, "dragon fruit": 3, "pineapple": 4}
    category2id = {sp[i][:-1]: i for i in range(len(sp))}
    categories = [

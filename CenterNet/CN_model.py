...
class CenterNet(nn.Module):
    ...
    def inference(self, img, topK=40, return_hm=False, th=None):
        ...
        for batch in range(b):
            ...
            batch_labels = clses[batch][mask]
            batch_clses = [self.CLASSES_NAME[int(cls.item())] for cls in batch_labels]

            detects.append([batch_boxes, batch_scores, batch_labels, batch_clses, pred_hm[batch] if return_hm else None])
        return detects
...
if __name__ == "__main__":
    # check model forward method
    labels_data = ["T-72", "Tiger", "Kamaz_kung", "BMP-3", "MTLBu", "KRAZ_FUEL", "BTR-82A", "UAZ", "URAL_gruz"]
    root_dir = r"C:\Users\ITC-Admin\PycharmProjects\Detection_military\datasets\fixed_kstovo_new"

    dataset_obj = CenterNetDataset(classes=labels_data, root_dir=root_dir)
    dataload = DataLoader(dataset_obj, batch_size=4, collate_fn=dataset_obj.collate_fn)
    ll = iter(dataload).__next__()
    cfg = Config
    model = CenterNet(cfg)
    x = ll[0]
    out_train = model(x)
    out_inference = model.inference(x)
